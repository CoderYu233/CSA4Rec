import numpy as np
import torch
import data_argument.utils
from data_argument.utils import timer
from collections import defaultdict
from torch.optim import Adam
from data_argument.utils import recall_at_k, ndcg_k, cal_mrr, get_user_performance_perpopularity
...
class Trainer:
    ...
    def precision(self,hits, N):
        prec = sum([hits[user] for user in hits])
        return round(prec / (len(hits) * N),5)

    def recall(self,hits):
        recall_list = [hits[user]/1 for user in hits]
        recall = round(sum(recall_list) / len(recall_list),5)
        return recall

    def hits(self,answers, pred_list):
        hit_count = {}
        for id, (ans, pre) in enumerate(zip(answers, pred_list)):
            items = list(ans)
            pred = list(pre)
            hit_count[id] = len(set(items).intersection(set(pred)))
        return hit_count

    #answer[user_len,1] pred_list[user_len,40]
    def get_full_sort_score(self, epoch, answers, pred_list):
        ...
        for k in [1, 5, 10, 15, 20, 40]:
            ...
            pred_n = pred_list[ : , : k]
            hits_k = self.hits(answers, pred_n)
            prec = self.precision(hits_k, k)
            recall_mine = self.recall(hits_k)
            recall_k.append(recall_mine)
            prec_k.append(prec)
        mrr, mrr_dict = cal_mrr(answers, pred_list)
        post_fix = {
            "Epoch": epoch,
            "HIT@1": '{:.8f}'.format(recall[0]), "NDCG@1": '{:.8f}'.format(ndcg[0]),"RECALL@1": '{:.8f}'.format(recall_k[0]), "PRECISION@1": '{:.8f}'.format(prec_k[0]),
            "HIT@5": '{:.8f}'.format(recall[1]), "NDCG@5": '{:.8f}'.format(ndcg[1]),"RECALL@5": '{:.8f}'.format(recall_k[1]), "PRECISION@5": '{:.8f}'.format(prec_k[1]),
            "HIT@10": '{:.8f}'.format(recall[2]), "NDCG@10": '{:.8f}'.format(ndcg[2]),"RECALL@10": '{:.8f}'.format(recall_k[2]), "PRECISION@10": '{:.8f}'.format(prec_k[2]),
            "HIT@15": '{:.8f}'.format(recall[3]), "NDCG@15": '{:.8f}'.format(ndcg[3]),"RECALL@15": '{:.8f}'.format(recall_k[3]), "PRECISION@15": '{:.8f}'.format(prec_k[3]),
            "HIT@20": '{:.8f}'.format(recall[4]), "NDCG@20": '{:.8f}'.format(ndcg[4]),"RECALL@20": '{:.8f}'.format(recall_k[4]), "PRECISION@20": '{:.8f}'.format(prec_k[4]),
            "HIT@40": '{:.8f}'.format(recall[5]), "NDCG@40": '{:.8f}'.format(ndcg[5]),"RECALL@40": '{:.8f}'.format(recall_k[5]), "PRECISION@40": '{:.8f}'.format(prec_k[5]),
            "MRR": '{:.8f}'.format(mrr)
        }
        print(post_fix, flush=True)
        return [recall[0], ndcg[0], recall[1], ndcg[1], recall[2], ndcg[2], recall[3], ndcg[3], recall[4], ndcg[4], recall[5], ndcg[5],
                recall_k[0], prec_k[0], recall_k[1], prec_k[1], recall_k[2], prec_k[2], recall_k[3], prec_k[3], recall_k[4], prec_k[4],recall_k[5], prec_k[5]
                   , mrr], str(post_fix), [recall_dict_list, ndcg_dict_list, mrr_dict]
    ...
    def iteration(self, epoch, full_sort=False, mode='train'):
        ...
        if mode=='train':
            ...
            with timer(name="Sample"):
                S = data_argument.utils.UniformSample_original(self.dataset)
            ...
            users, posItems, negItems = data_argument.utils.shuffle(users, posItems, negItems)
            total_batch = len(users) // self.args.bpr_batch + 1

            for (batch_i, (batch_users, batch_pos, batch_neg)) in enumerate(
                    data_argument.utils.minibatch(users, posItems, negItems, batch_size=self.args.bpr_batch)):
            ...
            with torch.no_grad():
                ...
                answer_list = None
                i = 0
                for batch_users in data_argument.utils.minibatch(users, batch_size=u_batch_size):
    ...
    def generateKorderGraph(self, userK, itemK, threshold=0.5):
        ...
        if itemK > 0:
            with torch.no_grad():
                ...
                for batch_items in data_argument.utils.minibatch(items, batch_size=u_batch_size):
